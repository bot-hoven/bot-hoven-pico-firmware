...
static const uint8_t LIMIT_PINS[] = {LL_LIMIT, LR_LIMIT, RL_LIMIT, RR_LIMIT};
...
void set_motor_pwm(uint slice, uint chan, uint8_t dir, float vel_mps) {
  gpio_put((slice == pwm_slice_l ? DIR_PIN_L : DIR_PIN_R), dir);
...
int main() {
  ...
  while (true) {
    // sleep_ms(2000);
    // printf("Current position: %d", step_count_r / STEPS_PER_METER);
    // set_motor_pwm(pwm_slice_r, pwm_chan_r, CCW, 0.1);
    // sleep_ms(5000);
    // set_motor_pwm(pwm_slice_r, pwm_chan_r, CW, 0.1);
    // sleep_ms(1000);
    // set_motor_pwm(pwm_slice_r, pwm_chan_r, CW, 0.0);
    // sleep_ms(500000);
