...
private:
    ...
    // Timer IRQ settings
    uint timer_block;        // Which timer block (0 or 1)
    uint alarm_num;          // Which alarm this stepper uses (0-3)
    bool using_timer_irq;    // Whether we're using timer IRQ
    
    // Static IRQ handlers (one per possible IRQ)
    static void timer_irq_0(void);
    static void timer_irq_1(void);
    static void timer_irq_2(void);
    static void timer_irq_3(void);
    static void timer_irq_4(void);
    static void timer_irq_5(void);
    static void timer_irq_6(void);
    static void timer_irq_7(void);

    // Helper function to get IRQ handler
    static irq_handler_t getIrqHandler(uint irq_num);

    // Static references to stepper instances (for IRQ handlers)
    static TrapezoidalStepper* stepper_instances[8];

    // Timer arm function
    void arm_timer(uint32_t us);
...
public:
     ...
     * @param stepsPerMeter Conversion factor: steps per meter of linear movement, will be recalculated during
     * calibration
     ...
     */
    TrapezoidalStepper(uint8_t pulsePin, uint8_t dirPin, int8_t enablePin, uint8_t leftLimitPin, uint8_t rightLimitPin,
                       uint32_t stepsPerMeter, float totalRailLength, float leftHandWidth, float rightHandWidth,
                       float homeOffset, uint32_t maxStepRate = 50000, uint32_t minPulseWidth = 2);

    // Initialize with timer IRQ
    bool initTimerIRQ(uint timer_block, uint alarm_num);

    // Handle a timer IRQ (called from static handler)
    void handleTimerIRQ();
